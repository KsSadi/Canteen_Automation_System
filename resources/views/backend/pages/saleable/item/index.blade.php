        ...
        <table class="table table-report -mt-2">
            ...
            @foreach ($sitems as $sitem)
                <tr class="intro-x">
                    ...
                    <td style="text-align:">
                        <span href="" class="font-medium whitespace-no-wrap">{{ $sitem->description }}</span>
                        {{--                        <div class="text-gray-600 text-xs whitespace-no-wrap"> <span class="badge badge bg-info">{{ $employee->post }}</span></div>--}}
                    </td>

                    <td style="text-align:">
                        <span href="" class="font-medium whitespace-no-wrap">{{ $sitem->scat_id }}</span>
                        {{--                        <div class="text-gray-600 text-xs whitespace-no-wrap"> <span class="badge badge bg-info">{{ $employee->post }}</span></div>--}}
                    </td>

                    <td style="text-align:">
                        <span href="" class="font-medium whitespace-no-wrap">{{ $sitem->sunit_id }}</span>
                        {{--                        <div class="text-gray-600 text-xs whitespace-no-wrap"> <span class="badge badge bg-info">{{ $employee->post }}</span></div>--}}
                    </td>
                    ...
                    <td style="text-align: center">
                        <div class="flex justify-center items-center" >
                            <a class="flex items-center mr-3" href="{{ route('dashboard.employees.edit', $sitem->id) }}">
                            ...
                            <a class="flex items-center text-theme-6" href="{{ route('dashboard.employees.destroy', $sitem->id) }}" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $sitem->id }}').submit()">
                            ...
                            <form id="delete-form-{{$sitem->id}}" action="{{ route('dashboard.employees.destroy', $sitem->id) }}" method="POST" style="display: none">
                                @method('DELETE')
                                @csrf
                            </form>
                        </div>
